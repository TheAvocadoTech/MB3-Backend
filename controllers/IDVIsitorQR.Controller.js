...
// ============================
// EMAIL HELPER FOR VISITOR PASS
// ============================
const sendVisitorPassEmail = async (visitor, qrCodeImage) => {
  if (!visitor.email) return;
  try {
    visitor.qrSentViaEmail = true;

    const pdfBuffer = await generateVisitorPDF(visitor, qrCodeImage);

    const mapUrl = `${process.env.FRONTEND_URL || "http://localhost:3000"}/navigation?token=${visitor.qrToken}`;
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 12px; background-color: #ffffff;">
          <h2 style="color: #1a237e; margin-top: 0; font-size: 20px;">Visitor Pass - ${visitor.visitorName}</h2>
          <p style="color: #374151; font-size: 14px;">Hello <strong>${visitor.visitorName}</strong>,</p>
          <p style="color: #4b5563; font-size: 14px;">Your digital visitor pass has been generated for your visit to <strong>${visitor.company || "our facility"}</strong>.</p>
          ...
          <p style="color: #4b5563; font-size: 13.5px; line-height: 1.5;">
            Present the attached PDF pass or scan your QR code at the reception. Scanning your QR code instantly opens the live floor map and indoor navigation to your destination.
          </p>

          <div style="margin-top: 24px; text-align: center;">
            <a href="${mapUrl}" style="display: inline-block; background: #2563eb; color: #ffffff; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 14px;">Open Live Navigation Map</a>
          </div>
        </div>
      `,
      attachments: [
...
// ===== CREATE OR UPDATE PERMANENT VISITOR =====
exports.createVisitor = async (req, res) => {
  try {
    const {
      ...
      purpose = "Meeting",
      expiryHours = 24,
      startVisit: shouldStartVisit = false,
    } = req.body;
    ...
    let visitor = await QRModel.findOne({
      $or: [{ phoneNumber: cleanPhone }, { email: cleanEmail }],
    });

    if (visitor) {
      // Update permanent master record details
      visitor.visitorName = cleanName;
      visitor.phoneNumber = cleanPhone;
    ...
    if ((shouldStartVisit || (company && (tagId || idNumber))) && !visitor.isOnVisit) {
      ...
      if (tagId && mongoose.Types.ObjectId.isValid(tagId)) {
        ...
        tag = await IdManagement.findOne({
          $or: [
            { macAddress: idNumber.toLowerCase() },
            { name: idNumber },
            { nickname: idNumber },
          ],
          isActive: true,
      ...
      if (tag) {
        ...
        // Mark tag as In Use
        tag.status = "In Use";
        tag.assignedVisitor = visitor._id;
        tag.assignedVisitorName = visitor.visitorName;
        tag.assignedAt = new Date();
        await tag.save();

        visitor.tagId = tag._id;
        visitor.tagNickname = tag.nickname;
        visitor.tagMac = tag.macAddress;
        visitor.idNumber = tag.name;
        visitor.mistMacAddress = tag.macAddress;
      } else if (idNumber) {
    ...
    res.status(201).json({
      success: true,
      message: visitor.isOnVisit
        ? "Visitor registered, visit started, and QR pass sent to email!"
        : "Permanent visitor profile saved successfully.",
      data: visitor,
    });
  } catch (error) {
...
exports.startVisit = async (req, res) => {
  try {
    ...
    if (!company) {
      return res.status(400).json({
        success: false,
        message: "Company is required to start a visit",
      });
    }
    ...
    // Resolve tag from ID Management DB
    let tag = null;
    if (tagId && mongoose.Types.ObjectId.isValid(tagId)) {
      tag = await IdManagement.findById(tagId);
    } else if (idNumber) {
    ...
    if (!tag && (tagId || idNumber)) {
      return res.status(404).json({
        success: false,
        message: "Selected Tag not found in Tag database.",
      });
    }
    ...
    // Automatically send QR Pass with PDF attachment to visitor's email
    await sendVisitorPassEmail(visitor, qrCodeImage);
    ...
    res.status(200).json({
      success: true,
      message: "Visit started, QR generated, and pass sent to visitor's email!",
      data: visitor,
    });
  } catch (error) {
...
exports.endVisit = async (req, res) => {
  try {
    ...
    // Free the assigned tag in ID Management
    if (visitor.tagId) {
      await IdManagement.findByIdAndUpdate(visitor.tagId, {
        status: "Available",
        assignedVisitor: null,
        assignedVisitorName: "",
        assignedAt: null,
      });
    } else if (visitor.tagMac || visitor.idNumber) {
      await IdManagement.updateMany(
        {
          $or: [
            { macAddress: (visitor.tagMac || "").toLowerCase() },
            { name: visitor.idNumber },
            { assignedVisitor: visitor._id },
          ],
        },
        {
          status: "Available",
  ...
  try {
    ...
    for (let i = 0; i < visitorsData.length; i++) {
      ...
      try {
        ...
        let visitor = await QRModel.findOne({ phoneNumber: cleanPhone });

        if (visitor) {
          visitor.visitorName = data.visitorName.trim();
          if (data.email) visitor.email = data.email.trim();
        } else {
          visitor = new QRModel({
    ...
    res.status(201).json({
      success: true,
      message: `Processed ${createdVisitors.length} visitors`,
...
exports.getAllVisitors = async (req, res) => {
  try {
    ...
    if (onVisitOnly === "true" || status === "onVisit" || status === "on_visit") {
      ...
      filter = { qrExpiresAt: { $gt: now }, checkedIn: true };
    ...
    if (search) {
      ...
      filter.$or = [
        ...
        { tagNickname: searchRegex },
      ];
    }

    const [visitors, total] = await Promise.all([
      QRModel.find(filter)
        .populate("tagId", "nickname name macAddress status")
        .sort({ isOnVisit: -1, updatedAt: -1 })
        .skip(skip)
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 12px; background-color: #ffffff;">
          <h2 style="color: #1a237e; margin-top: 0; font-size: 20px;">Visitor Pass - ${visitor.visitorName}</h2>
          <p style="color: #374151; font-size: 14px;">Hello <strong>${visitor.visitorName}</strong>,</p>
          <p style="color: #4b5563; font-size: 14px;">Your digital visitor pass has been resent for your visit to <strong>${visitor.company || "our facility"}</strong>.</p>
...
// ===== CHECK-OUT / END VISIT VISITOR =====
exports.checkOutVisitor = async (req, res) => {
  return exports.endVisit(req, res);
};
